...
namespace YourWebApiName.ApiServices.RestApi.v1.sys
{
    ...
    public class SysMenusController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery]PagingModel paging, [FromQuery]SysMenusRequestModel queryParameter)
        {
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(string id, [FromBody]SysMenusRequestModel parameter)
        {
            var apiResult = new ApiResultModel(ErrorCodeType.Success);
            var model = new SysMenusModel();
            parameter.CloneTo(model);
            var c = await sysMenusService.UpdateModelAsync(id, model);
            if (c > 0)
            {
                return Ok(apiResult);
            }
