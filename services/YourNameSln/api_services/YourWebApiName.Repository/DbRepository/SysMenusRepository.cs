    ...
    public class SysMenusRepository : ISysMenusRepository
    {
       ...
       public SysMenusRepository()
       {
        ...
        public async Task<IEnumerable<SysMenusModel>> GetCurrentModelsAsync(SysMenusRequestModel queryParameter)
        {
            ...
            var dataQuery = $"SELECT b1.* FROM {tableName} b1 WHERE 1=1 {strWhere} ORDER BY menu_id ASC";
        ...
        {
            var strWhere = GetQuery(queryParameter);
            var dataQuery = $"SELECT b1_result.* FROM (SELECT b1.* FROM {tableName}  b1 WHERE 1=1 {strWhere} ORDER BY menu_id ASC) b1_result";//内查询，可以做连接查询 直接join
        ...
        {
            var strWhere = GetQuery(queryParameter);
            var querySql = "SELECT {0} " + $"FROM (SELECT b1.* FROM {tableName}  b1 WHERE 1=1 {strWhere} ORDER BY menu_id ASC) b1_result";//内查询，可以做连接查询 直接join
