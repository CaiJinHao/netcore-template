    ...
    public class SysUsersRepository : ISysUsersRepository
    {
       ...
       public SysUsersRepository()
       {
        ...
        public async Task<IEnumerable<SysUsersModel>> GetCurrentModelsAsync(SysUsersRequestModel queryParameter)
        {
            ...
            var dataQuery = $"SELECT b1.* FROM {tableName} b1 WHERE 1=1 {strWhere} ORDER BY b1.user_id ASC";
        ...
        {
            var strWhere = GetQuery(queryParameter);
            var dataQuery = $"SELECT b1_result.* FROM (SELECT b1.* FROM {tableName}  b1 WHERE 1=1 {strWhere}) b1_result ORDER BY b1_result.user_id ASC";//内查询，可以做连接查询 直接join
        ...
        public async Task<IEnumerable<SysUsersResponeModel>> GetModelsAsync(PagingModel pagingModel, SysUsersRequestModel queryParameter)
        {
            ...
            var pagingQuerySql = string.Format(querySql, "ROW_NUMBER() OVER(ORDER BY b1_result.user_id ASC) AS RowNum,b1_result.*");//按带索引的字段排序，否则很慢
